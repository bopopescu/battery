...
class Oven(object):
    ...
    def __init__(self):
        self.port = serial.Serial(port='COM3', baudrate=9600, bytesize=serial.EIGHTBITS, parity=serial.PARITY_NONE,
    ...
    def buildcmd(self, name="SV", mode="read", value=0x0000):
        if name not in self.valDict8.keys():
            name = "SV"
            print("没有该参数值")
            return None
        if mode == "read":
            checksum = self.valDict8[name] * 256 + 82 + self.addr
            checksumLO = (checksum & 0x00ff)
            checksumHI = (checksum & 0xff00) >> 8
            cmd = [0x80 + self.addr, 0x80 + self.addr,
                   0x52, self.valDict8[name],
                   0x00, 0x00,
                   checksumLO, checksumHI]
            cmd = bytearray(cmd)
            return cmd
            ...
            valueHI = (value & 0xff00) >> 8
            valueLO = (value & 0x00ff)
            checksum = self.valDict8[name] * 256 + 67 + value + self.addr
            checksumLO = (checksum & 0x00ff)
            checksumHI = (checksum & 0xff00) >> 8
            cmd = [0x80 + self.addr, 0x80 + self.addr,
                   0x43, self.valDict8[name],
                   valueLO, valueHI,
                   checksumLO, checksumHI]
            cmd = bytearray(cmd)
            return cmd
...
if __name__ == "__main__":
    ...
    #data = oven.sendcmd(oven.buildcmdx(addr=22, mode="set",value=1))
    #print(str(22) + ":" + str(data))
    # oven.setPlan(steps)
    # oven.startOven()
    # print(oven.sendcmd(oven.buildcmd("SV/SteP","read")))
    data = oven.sendcmd(oven.buildcmd(name='Srun', mode="set",value=1))
    print( "Srun:" + str(data))
    for keys in oven.valDict8:
        data = oven.sendcmd(oven.buildcmd(name=keys, mode="read"))
        print(keys + ":" + str(data))
    # for i in range(0,150):
    #     data = oven.sendcmd(oven.buildcmdx(addr=i, mode="read"))
